...
app.controller("UserManagementController", function ($scope, $http) {
    ...
    $scope.submitPopulationL = function () {
        $http({
            method: "POST",
            url: '/projects/addressbookdatabase/population',
    ...
    $scope.submitName = function () {
        if ($scope.name != "") {
            $http({
                method: "POST",
                url: '/projects/addressbookdatabase/name',
                data: $scope.name,
                headers: {
                    'Content-Type': 'text/plain'
                }
    ...
    $scope.submitRecord = function () {
        $http({
            method: "POST",
            url: '/projects/addressbookdatabase/continent',
            data: $scope.record,
            headers: {
                'Content-Type': 'text/plain'
            }
    ...
    function _getAllContinents() {
        $http({
            method: 'GET',
            url: '/projects/addressbookdatabase/continents'
    ...
    $scope.submitCountry = function () {
        ...
        if ($scope.form.id == -1) {
            ...
            method = "POST";
            url = '/projects/addressbookdatabase/countries';
        } else {
            //If Id is present, it's edit operation - PUT operation
            method = "PUT";
            url = '/projects/addressbookdatabase/countries/' + $scope.form.id;
        }
    ...
    $scope.removeCountry = function (country) {
        $http({
            method: 'DELETE',
            url: '/projects/addressbookdatabase/countries/' + country.id
        }).then(_success, _error);
    };
